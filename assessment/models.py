...
class DiagnosticSession(models.Model):
    ...
    def has_converged(self):
        return self.current_se < getattr(settings, 'IRT_CONVERGENCE_THRESHOLD', 0.3)

    @property
    def should_terminate(self):
        max_questions = getattr(settings, 'IRT_MAX_QUESTIONS', 30)
        return self.has_converged or self.question_count >= max_questions
...
class QuestionBank(models.Model):
    ...
    guessing_c = models.FloatField(default=0.25, validators=[MinValueValidator(0.0), MaxValueValidator(1.0)])
...
class AnswerLog(models.Model):
    ...
    question = models.ForeignKey(QuestionBank, on_delete=models.CASCADE, related_name='answer_logs')
    user_answer = models.IntegerField(validators=[MinValueValidator(0), MaxValueValidator(3)])
...
class SkillGap(models.Model):
    ...
    occupation = models.ForeignKey('skills.Occupation', on_delete=models.CASCADE, related_name='user_gaps')
    skill = models.ForeignKey('skills.Skill', on_delete=models.CASCADE, related_name='user_gaps')
    ...
    criticality_coefficient = models.FloatField(default=1.0, help_text='Weight based on importance')
